...
(function() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (i < lines.length - 1 && line.startsWith('#EXTINF') && (!line.includes(',live') || stripAllSegments || AllSegmentsAreAdSegments || inCueOut)) {
                ...
                if (line.startsWith('#EXT-X-TWITCH-PREFETCH:')) {
                    ...
                    const hintMatch = line.match(URI_ATTRIBUTE_REGEX);
    ...
    function reloadTwitchPlayer(isPausePlay, reloadKind) {
        ...
        // Reload cooldown — early reloads bypass the gate but still update bookkeeping so
        // 3+ rapid early reloads still trigger the 90s auto-escalation.
        ...
        try {
            ...
            if (v && !v.muted) {
                v.muted = true;
                // Multi-event restore — issue #200 follow-up.
                let done = false;
                const restore = () => {
                    if (done) return;
                    done = true;
