    ...
    function updateAdblockBanner(data) {
        ...
        if (playerRootDiv != null) {
            ...
            if (adBlockDiv != null) {
                ...
                adBlockDiv.P.textContent = 'Blocking' + (data.isMidroll ? ' midroll' : '') + ' ads' + (data.isStrippingAdSegments ? ' (stripping)' : '') + (data.activeBackupPlayerType && data.activeBackupPlayerType !== BackupPlayerTypes[0] ? ' (' + data.activeBackupPlayerType + ')' : '');// + (data.numStrippedAdSegments > 0 ? ` (${data.numStrippedAdSegments})` : '');
