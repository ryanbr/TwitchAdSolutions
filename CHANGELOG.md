...
### Added
- **Separate Twitch video ads beside the player and in chat are now hidden** — since ~July 2026 Twitch renders standalone `<video>` ad elements next to the stream and in chat (the "second player with a Play-ad button"). They are delivered *outside* the m3u8, so neither segment stripping nor backup-swapping touches them. The existing `hideTwitchAdOverlays()` tick now also matches `<video>` elements whose source is on the Amazon ad CDN (`media-amazon.com`) and hides, mutes and pauses them — a `display:none` `<video>` still plays audio. Matching is on the ad-CDN host alone, which cannot false-positive on the live stream: MediaSource always gives the primary player a `blob:` URL. The player-owned element is skipped as a second independent guard, and recycled elements are restored when their source is no longer an ad. **Known limitation:** page-context scripts cannot reach cross-origin iframes, so if Twitch renders these ads inside an iframe this guard will not see them — unconfirmed either way. Mirrors GosuDRM/TTV-AB v12.0.1–12.0.8. vaft only (#249)

### Fixed
- **Video lookups no longer resolve to an ad element** — `document.querySelector('video')` returns the *first* `<video>` in the DOM, which since the change above can be a side or chat ad. Five sites relied on it, including the unmute after a hard reload, the silent-re-mute backstop predicate, and both drift-correction paths — so mute and `playbackRate` work could land on an ad element, or be skipped because of one, instead of reaching the real player. All five now go through a `getPlayerVideoElement()` helper that skips elements the ad guard has marked. vaft only (#249)
- **The ad-element marker never registered** — `dataset.tasAdHidden` was assigned `''`, which `dataset` reads back as the empty string and is therefore falsy. The once-only log deduped nothing and fired on every buffer-monitor tick, the restore-recycled-`<video>` branch was unreachable dead code (a recycled element would have stayed hidden and muted permanently), and the guards that consulted the marker never guarded. Assigns `'1'`. vaft only (#249)
- **AV1 streams never reloaded onto the AVC swap playlist** — v68.5.0 added AV1 (`av0*`) to the ModifiedM3U8 build on the *master*-playlist side, but the *media*-playlist reload trigger still matched only `hev`/`hvc`. A native-AV1 stream therefore built the AVC swap and then never switched to it, playing AV1 straight into the strip path — the exact black screen v68.5.0 set out to fix, on the other enhanced family. The trigger now covers both families; `isHevc`/`stripHevc` renamed `isEnhanced`/`stripEnhanced` since the names no longer described what they gate. `SkipPlayerReloadOnHevc` keeps its name and now gates both. vaft only
- **A worker blob already carrying our hooks is no longer re-injected** — the Twitch-worker test accepts any `blob:` URL whose origin is twitch.tv, which includes the blob vaft creates itself. If such a URL re-entered the constructor — most plausibly another `Worker` wrapper delegating through, which happens when more than one Twitch ad blocker is active — the hooks were installed a second time on top of the first: duplicate `hookWorkerFetch`, duplicate m3u8 processing, duplicate `UpdateAdBlockBanner` messages. The already-fetched worker source is now tested for our own marker and passed through untouched when it matches. Prophylactic — no field report behind it. Applies to vaft, video-swap-new and strip. Prior art: GosuDRM/TTV-AB 16.2.3/16.2.4 (GosuDRM/TTV-AB#63)
- **iOS/iPadOS stayed pinned at 360p after an autoplay escape-hatch break** — `setSrc()` took `isNewMediaPlayerInstance` and `refreshAccessToken` from the same flag. On Apple touch devices `iosSoftReload` forces that flag false to keep the media element user-gesture-blessed (the v68.4.0 black-screen fix), which also silently dropped the token refresh. An `'early'` reload after an autoplay commit relies on that refresh to leave the autoplay-scoped 360p variant ladder; since `'early'` is always downgraded on iOS and `'post-ad'` is soft by definition, nothing refreshed the token again. The refresh now follows the requested reload kind, independent of whether the media instance is recreated — so the gesture blessing is preserved and Source quality can still be restored. The `setSrc` teardown-pause filter is armed on the same path, so the swap cannot be misread as user pause intent. **Desktop is bit-for-bit unchanged.** vaft only
- **A master-playlist variant without a `CODECS` attribute no longer throws** — the codec tests dereferenced `element.Codecs.startsWith(...)` directly, and `CODECS` is optional on a `STREAM-INF` line while the entry is recorded whenever `RESOLUTION` is present. All four codec checks now go through the `videoCodecFamily()` classifier already used in the testing scripts, and `Codecs` gained the `|| ''` fallback its `Audio`/`Video`/`Subtitles` neighbours already had. Also corrects two classification bugs that did not throw: the check was case-sensitive and only ever inspected the start of the whole attribute, so an audio-first `CODECS` value classified as non-AVC. Twitch emits video-first, lowercase prefixes and always includes `CODECS`, so this is hygiene and parity with the testing scripts rather than a fix for anything observed in the field. vaft only
