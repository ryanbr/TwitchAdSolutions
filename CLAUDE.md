...
- `twitchAdSolutions_vodCsaiBypass` — opt-in via `'true'` (default OFF). **Main-thread only, NOT injected into the worker blob** (the `edge.ads.twitch.tv` hooks live in `hookFetch` and the `XMLHttpRequest.prototype.open` hook, both main-thread). Converts those log-only detection sites into a `200 OK` empty-ad-list (`[]`) stub, so the ad orchestrator never enters the pending-ad-break state that produces VOD freeze/black-screen. Fetch path returns `realFetch('data:application/json,%5B%5D')` (a real response with populated `.url`/`.type`/`.redirected` — avoids the IVS-WASM `NetworkError` that a bare `new Response()` with empty `url` can trigger, same class as the worker-relay v6.3.5 fix). XHR path redirects to `data:application/json;base64,W10=` (base64 of `[]`) and no-ops `setRequestHeader` on the stubbed instance. **Host-based match → stubs ALL CSAI ad-decisions (VOD + live CSAI), not VOD-only.** Sub-flag `twitchAdSolutions_vodHideOverlay` (default ON when master is ON; `'false'` to disable while keeping the network stub) injects a one-shot `<style>` hiding `[data-a-target="ax-overlay"]`, `[data-a-target="video-ad-label"]`, `[data-test-selector="ad-banner-default-id"]` — exact ad-specific attribute selectors, no parent walking. Default code path byte-for-byte unchanged when unset. vaft only.
