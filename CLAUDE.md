...
- `twitchAdSolutions_vodCsaiBypass` — opt-in via `'true'` (default OFF). Converts the existing `edge.ads.twitch.tv` detection sites (in `hookFetch` ~L2696 and the `XMLHttpRequest.prototype.open` hook ~L2839) from log-only into a `200 OK` empty-ad-list (`[]`) stub. Defeats VOD pre-roll/mid-roll ad-break entry without touching the VOD m3u8 — the orchestrator never enters the pending-ad-break state that otherwise produces the freeze/black-screen issues reported on VOD pages (upstream #442, #473, #278). XHR site redirects to `data:application/json;base64,Wwo=` so the request completes naturally with `[]`; `setRequestHeader` calls on stubbed XHRs are no-oped (data: URLs reject custom headers). Sub-flag `twitchAdSolutions_vodHideOverlay` (default ON when master is ON; set to `'false'` to disable while keeping the network stub) injects a one-shot `<style>` rule hiding `[data-a-target="ax-overlay"]`, `[data-a-target="video-ad-label"]`, `[data-test-selector="ad-banner-default-id"]` — stable data-attribute selectors only, no parent walking (consistent with the overlay-hide policy in this file). Live-stream code path is byte-for-byte unchanged when master flag is off. vaft only.
